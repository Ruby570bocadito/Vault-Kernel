...
const statsReportFixture = "module=vault_kernel version=3.12\n" +
...
func TestParseStatsReportKeepsAllPairsOfALine(t *testing.T) {
	got := ParseStatsReport("module=vault_kernel version=3.12\n")
	if got["module"] != "vault_kernel" {
		t.Errorf("module = %q, want %q", got["module"], "vault_kernel")
	}
	if got["version"] != "3.12" {
		t.Errorf("version = %q, want %q (line parser lost this key)", got["version"], "3.12")
	}
}
